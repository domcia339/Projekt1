...
class DAP:
    ...
    def lista_mozliwych_genow(self, n, total):
    ...
    def generate_list_of_gens_for_all_demands(self, D, m_d, h_d):
        gens=[]
        for i in range(0, D):
            lista = self.lista_mozliwych_genow(m_d[i], h_d[i])
            gens.append(lista)
        return gens

    # Generuje tablice która ustala indeksy genow jakie będą wczytane do chromosomów
    def generate_possible_chromosoms(self, D, numberOfPossibleGens ):
    ...
    def generate_possible_chromosoms_BIG(self, D, numberOfPossibleGens ):
    ...
    def generate_list_of_possible_chromosoms(self, D, numberOfPossibleGens, possibleGens):

        if D < 20:
            possibleChromosoms= self.generate_possible_chromosoms(D, numberOfPossibleGens)
        else:
            possibleChromosoms = self.generate_possible_chromosoms_BIG(D, numberOfPossibleGens)
    ...
    def minimize_function(self, tableOfChromosom ):

        f_x = []
        for chrom in tableOfChromosom:
            link_loads = []

            for item in range(1, self.data.E+1):
                ll = 0
                for i in self.data.P:
                    ii = self.data.P.index(i)
            ...
            for i in range(len(link_loads)):
                l_e_x.append(max(0, link_loads[i] - self.data.links[0][2]*self.data.links[0][4]))
    ...
    def suma_elementow_listy(self, minimize_f_x):

        suma_minimize_f_x = []
        for i in minimize_f_x:
            suma_minimize_f_x.append(sum(i))

        return suma_minimize_f_x
